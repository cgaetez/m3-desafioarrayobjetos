...
//1. Agregar las siguientes horas al arreglo de Traumatología
    ...
    console.log('trauma');
    for(let item in traumatologia){
        console.log(traumatologia[item]);
    }
...
//2. Eliminar el primer y último elemento del arreglo de Radiología 
    ...
    radiologia.pop();
    console.log('radio');
    for(let item in radiologia){
        console.log(radiologia[item]);
    }
...
/* 3 Imprimir en la página HTML, mediante document.write y/o las funciones que estime
    ...
    document.write('<h2>Horas Dentales</h2>');
    for (let item of dental) {
        document.write(`<p>${item.hora} - ${item.especialista} - ${item.paciente} - ${item.rut} - ${item.prevision}   </p>`);
    }
...
/*    4. Imprimir un listado total de todos los pacientes que se atendieron en el centro médico.
      ...
      document.write('<h2>Pacientes</h2>');  
      for (let item of traumatologia) {
        document.write(`<p>${item.paciente}</p>`);
      }
      for (let item of radiologia) {
        document.write(`<p>${item.paciente}</p>`);
      }
      for (let item of dental) {
        document.write(`<p>${item.paciente}</p>`);
